...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 492 $")
...
/* CEPSTRAL_CHANGE: End new code by Cepstral */		
	...
	if (max_digits >= 1 && results != NULL) {
		if (cfg_goto_exten) {
		  ...
		  ///ast_party_caller c = ast_channel_caller(chan);
		  if (ast_exists_extension (chan, ast_channel_context(chan), results, 1, ast_channel_caller(chan)->id.number.str)){//->id.number.str)){
				ast_log(LOG_NOTICE, "OK\n");
				ast_copy_string(ast_channel_exten(chan), results, sizeof(ast_channel_exten(chan) - 1));
